...
class WithdrawWindow(QDialog):
    ...
    def init_ui(self):
        self.setWindowTitle("Withdraw Account")
        self.setGeometry(100, 100, 300, 200)

        self.withdraw_account_button = QPushButton("Withdraw account")
        ...
        self.text_input_label = QLabel('Entry withdraw')
    ...
    def withdraw_account_button_clicked(self):
        ...
        selected_account = Function.accounts[index - 1]['account']
        success = Function.withdraw_money(selected_account, amount)

        if success:
            Function.save_accounts_to_file(Function.accounts)
            self.update_balance_label(index)
            self.text_input.setText(str(0))
            QMessageBox.information(self, "Success", f"Withdrawal of {amount} euro was successful")
        else:
            QMessageBox.warning(self, "Warning", "Not enough balance")
